...
class DataManager:
    ...
    def export_training_data(self, export_path: str, format_type: str = 'json') -> bool:
        """
        导出训练数据
        ...
        try:
            if format_type == 'json':
                ...
                return self._export_xml(export_path)
            else:
                self.gui.log_error(f"不支持的导出格式: {format_type}", "DATA")
                return False
                
        except Exception as e:
            self.gui.log_error(f"导出数据失败: {e}", "DATA")
            return False
    ...
    def _export_csv(self, export_path: str) -> bool:
        ...
        with open(export_path, 'w', newline='', encoding='utf-8') as f:
            ...
            # 写入表头
            headers = ['panoramic_id', 'hole_number', 'growth_level', 'confidence', 
                      'microbe_type', 'interference_factors', 'annotation_source', 
                      'is_confirmed', 'image_path', 'timestamp']
            ...
            for panoramic_annotations in self.annotations.values():
                for annotation in panoramic_annotations.values():
                    row = [
                        annotation.panoramic_id,
                        annotation.hole_number,
                        annotation.growth_level,
                        annotation.confidence,
                        annotation.microbe_type,
                        ';'.join(annotation.interference_factors),
    ...
    def _export_xml(self, export_path: str) -> bool:
        """导出为XML格式"""
        ...
        for panoramic_annotations in self.annotations.values():
            for annotation in panoramic_annotations.values():
                ...
                for field, value in asdict(annotation).items():
                    field_elem = ET.SubElement(ann_elem, field)
                    if isinstance(value, list):
                        field_elem.text = ';'.join(value)
                    else:
