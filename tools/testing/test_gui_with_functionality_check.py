...
def run_panoramic_workflow_check(test_image_dir: str = "D:\\test\\images") -> bool:
    """Run panoramic workflow check before GUI launch"""
    print("🎯 Running Panoramic Workflow Check...")
    print("=" * 50)
    ...
    try:
        ...
        print("   Testing panoramic annotation workflow...")
        success = run_comprehensive_workflow_test(test_image_dir)
        
        if success:
            print("✅ Panoramic workflow check PASSED")
            print("   Panoramic annotation system is ready")
            return True
        else:
            print("❌ Panoramic workflow check FAILED")
            print("   Panoramic annotation system has issues")
            return False
            
    except Exception as e:
        print(f"⚠️ Panoramic workflow check failed with error: {e}")
        return False
...
def main():
    ...
    parser.add_argument(
        '--panoramic-workflow',
        action='store_true',
        help='Run panoramic workflow tests instead of general functionality tests'
    )
    ...
    try:
        if args.test_only:
            # Run tests only
            if args.panoramic_workflow:
                success = run_panoramic_workflow_check(args.test_dir)
            else:
            ...
            if args.panoramic_workflow:
                test_passed = run_panoramic_workflow_check(args.test_dir)
            else:
